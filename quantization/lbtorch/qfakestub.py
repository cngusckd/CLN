...
from lbtorch.functional import quant, dequant
...
class QFakeStub(nn.Module):
    ...
    def update_qparams(self):
        max_val_pos = torch.max(self.observer.max_val, torch.zeros(1, device=self.observer.max_val.device))
        min_val_neg = torch.min(self.observer.min_val, torch.zeros(1, device=self.observer.min_val.device))
        scale = (max_val_pos - min_val_neg) / float(self.quant_max - self.quant_min)
        self.eps = self.eps.to(scale.device)  # self.eps를 scale과 동일한 디바이스로 이동
